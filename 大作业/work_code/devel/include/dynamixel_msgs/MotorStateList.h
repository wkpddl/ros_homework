...
struct MotorStateList_
{
  ...
  (void)_alloc;
   ...
   typedef std::vector< ::dynamixel_msgs::MotorState_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::dynamixel_msgs::MotorState_<ContainerAllocator> >> _motor_states_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::dynamixel_msgs::MotorStateList_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.motor_states.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::dynamixel_msgs::MotorState_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.motor_states[i]);
    }
    if (v.motor_states.empty() || false)
      s << "]";
  }
