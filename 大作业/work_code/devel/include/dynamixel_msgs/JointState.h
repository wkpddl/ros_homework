...
struct JointState_
{
  ...
  _name_type name;
   ...
   typedef std::vector<int32_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<int32_t>> _motor_ids_type;
  _motor_ids_type motor_ids;

   typedef std::vector<int32_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<int32_t>> _motor_temps_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::dynamixel_msgs::JointState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.motor_ids.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.motor_ids[i]);
    }
    if (v.motor_ids.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "motor_temps: ";
    if (v.motor_temps.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.motor_temps.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.motor_temps[i]);
    }
    if (v.motor_temps.empty() || true)
      s << "]";
    if (true || !indent.empty())
